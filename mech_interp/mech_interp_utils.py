...
def get_label(query: dict, threshold: int = 3, consider_refusal_label: bool = False):
    ...
    total_correct = sum(
        [
            ...
            for label in query["string_matching_sampled_labels"]
        ]
    )
    return 1 if total_correct >= threshold else 0


def get_known_unknown_splits(
    queries: list,
    min_correct: int = 1,
    return_type: Literal["entity", "prompt"] = "entity",
    split: str = "train",
    consider_refusal_label: bool = False,
):
    """
    ...
    if return_type == "entity":
        ...
        ), "Refusal label is not supported when working with entities"
    for query in queries:
        if query[return_type] not in results.keys():
            results[query[return_type]] = 0
        results[query[return_type]] += get_label(
            query, consider_refusal_label=consider_refusal_label
        )
    ...
    train_known, test_known = train_test_split(
        known, test_size=0.5, random_state=random_seed
    )
    train_unknown, test_unknown = train_test_split(
        unknown, test_size=0.5, random_state=random_seed
    )
    # Create validation split
    val_known, test_known = train_test_split(
        test_known, test_size=0.8, random_state=random_seed
    )
    val_unknown, test_unknown = train_test_split(
        test_unknown, test_size=0.8, random_state=random_seed
    )
...
def get_valid_entities(
    queries, tokenizer, entity_type, entity_token_pos, split="train", fixed_length=False
):
    print("queries", len(queries))
    queries = [query for query in queries if query["entity_type"] == entity_type]
    known_entities, unknown_entities = get_known_unknown_splits(
        queries, return_type="entity", split=split
    )

    valid_entities = defaultdict(list)
    for known_label, entities in zip(
        ["known", "unknown"], [known_entities, unknown_entities]
    ):
        for entity in entities:
            tokenized_entity = torch.tensor(
                tokenizer.encode(f"The {entity_type} {entity}")
            ).to("cuda")
            if fixed_length:
...
def create_prompts_and_answers(
    tokenizer,
    queries,
    entity_type,
    known_label,
    valid_entities,
    prompt_template,
    relations_model_type: Literal["base", "it"] = "base",
    fixed_relation_length: bool = False,
):
    ...
    entity_type_to_relations = {
        key: value
        for (key, value) in entity_type_to_relations.items()
    ...
    if fixed_relation_length:
        ...
        for prompt_type, relation in entity_type_to_relations.items():
            ...
            tokenized_realation = torch.tensor(tokenizer.encode(f"The {relation}")).to(
                "cuda"
            )
    ...
    entity_type_relations = [
        valid_relations[key] for key in valid_relations if key.startswith(entity_type)
    ]
    ...
    for entity in valid_entities[known_label]:
        for relation in entity_type_relations:
            prompts_label.append(
                prompt_template.format(
                    entity_name=entity, relation=relation, entity_type=entity_type
                )
            )
            for query in queries:
                if (
                    query["entity"] == entity
                    and query["prompt_type"] == relations_to_entity_type[relation]
                ):
                    answers_label.append(
                        " " + query["correct_answer"].split(",")[-1].strip()
                    )
    ...
    prompts_label, answers_label, entities_label = shuffle_lists(
        prompts_label, answers_label, entities_label
    )
...
def load_data(
    ...
    fixed_config: Optional[Literal["fixed_entities", "both"]] = None,
):
    ...
    valid_entities = get_valid_entities(
        ...
        entity_token_pos,
        split=split,
        fixed_length=fixed_entities_length,
    )
    prompts_label, answers_label, entities_label = create_prompts_and_answers(
        tokenizer,
        copy.deepcopy(queries),
        entity_type,
        known_label,
    ...
    if relations_model_type == "it":
        ...
        for prompt in prompts_label:
            ...
            formatted_instructions.append(
                tokenizer.apply_chat_template(conversation, tokenize=False).replace(
                    tokenizer.bos_token, ""
                )[: -len("<end_of_turn>\n")]
            )
    else:
    ...
    for i in range(len(tokenized_prompts)):
        ...
        slice_entity_pos = find_string_in_tokens(
            entity, tokenized_prompts[i], tokenizer
        )
...
def get_attn_to_entity_tok_mean(
    model,
    cache,
    entity_token_pos,
    n_layers,
    type_pattern: Literal["attn_weights", "value_weighted"] = "attn_weights",
):
    ...
    for layer in range(n_layers):
        if type_pattern == "attn_weights":
            ...
            attn_to_entity_toks_sum = torch.zeros(
                attn_to_entity_toks.shape[0], attn_patterns.shape[1]
            )
    ...
    attn_to_entity_tok_stack = einops.rearrange(
        attn_to_entity_tok_stack, "layers batch heads -> batch layers heads"
    )
...
def load_steering_latents(
    ...
    if specific_latents is not None:
        ...
        for layer, latent_idx in specific_latents:
            for sae_latent_idx in all_sae_latents_dict[label].keys():
                if (
                    all_sae_latents_dict[label][sae_latent_idx]["layer"] == layer
                    and all_sae_latents_dict[label][sae_latent_idx]["latent_idx"]
                    == latent_idx
                ):
                    indices.append(sae_latent_idx)
        sae_latent_dict = {
            i: all_sae_latents_dict[label][idx] for i, idx in enumerate(indices)
        }
    ...
    for layer in layers:
        if model_alias == "gemma-2b-it":
            ...
            or model_alias == "meta-llama_Llama-3.1-8B"
        ):
            ...
            assert layer in [
                ...
                21,
                32,
            ], "Layer 10, 21, 32 are the only layers for gemma-2-9b-it"
...
def get_prompt_from_ids(model_alias, input_ids, tokenizer):
    ...
    if "gemma" in model_alias.lower():
        cleaned_decoded_strings = [
            decoded_string.replace("<bos>", "")
            .replace("<eos>", "")
            .replace("<pad>", "")
        ...
        cleaned_decoded_strings = [
            decoded_string.replace("<|end_of_text|>", "").replace(
                "<|begin_of_text|>", ""
            )
            for decoded_string in decoded_strings
        ]
...
def get_acts_labels_dict(
    model_alias,
    tokenizer,
    dataloader,
    queries: List[Dict],
    prompts: List[str],
    known_prompts_or_entities: List[str],
    unknown_prompts_or_entities: List[str],
    input_data_type: Literal["prompts", "entities", "bias"],
    layers: List[int],
    ...
    for i, original_prompt_text in enumerate(
        prompts
    ):  # 'prompts' here is the original list from outside
        cleaned_original_prompt_text = (
            original_prompt_text.replace("<bos>", "")
            if original_prompt_text.startswith("<bos>")
            else original_prompt_text
        )
        # Apply Gemma-specific EOI truncation if this is applicable to how `prompts` were generated/truncated during caching
        if (
            "gemma" in model_alias.lower() and input_data_type == "prompts"
        ):  # Assuming this condition for Gemma
            eoi_index = cleaned_original_prompt_text.find(eoi_string)
            if eoi_index != -1:
                cleaned_original_prompt_text = cleaned_original_prompt_text[
                    : eoi_index + len(eoi_string)
                ]

        # We need the full query object, so map to queries[i]
        query_map_by_cleaned_prompt_string[cleaned_original_prompt_text] = queries[
            i
        ]  # queries is the original list of dicts
    ...
    if input_data_type == "prompts":
        ...
        known_prompts = set(
            [
                prompt.replace("<bos>", "") if prompt.startswith("<bos>") else prompt
                for prompt in known_prompts_or_entities
            ]
        )
        unknown_prompts = set(
            [
                prompt.replace("<bos>", "") if prompt.startswith("<bos>") else prompt
                for prompt in unknown_prompts_or_entities
            ]
        )
    elif input_data_type == "entities" or input_data_type == "bias":
        known_entities = set(known_prompts_or_entities)
        unknown_entities = set(
            unknown_prompts_or_entities
        )  # These are NOT USED in the fixed bias block
    ...
    for _, (batch_activations, batch_input_ids) in tqdm(
        ...
        batch_cleaned_decoded_strings = get_prompt_from_ids(
            model_alias, batch_input_ids, tokenizer
        )

        for j, clean_string_from_cache in enumerate(
            batch_cleaned_decoded_strings
        ):  # Renamed `clean_string` to `clean_string_from_cache` for clarity
            ...
            if input_data_type == "prompts":
                if "gemma" in model_alias.lower():
                    ...
                    if eoi_index != -1:
                        clean_string_from_cache = clean_string_from_cache[
                            : clean_string_from_cache.find(eoi_string) + len(eoi_string)
                        ]
            ...
            original_query = query_map_by_cleaned_prompt_string.get(
                clean_string_from_cache
            )
            ...
            if input_data_type == "prompts":
                ...
                bias_cat = original_query.get(
                    "bias_cat"
                )  # Safely get 'bias_cat' from the found original_query
    ...
    if not activations_list:
        print(
            f"Warning: No valid activations collected for dataset {input_data_type}. Returning empty dict."
        )
        return {}
    ...
    all_collected_activations = torch.stack(
        activations_list, dim=0
    )  # Shape (num_collected_samples, n_layers, d_model)
    ...
    for layer_idx, layer_id in enumerate(layers):  # Iterate through your desired layers
        ...
        acts_for_this_layer = activations_to_use[
            :, layer_id, :
        ]  # Resulting shape: (num_samples, d_model)
...
def get_features_layers(
    model_alias, acts_labels_dict, layers, sae_width, repo_id, save=True, **kwargs
):
    """
    ...
    for layer in layers:
        ...
        scores_dict, freq_acts_dict, mean_features_acts = get_features(
            sae_acts,
    ...
    if save == True:
        ...
        filename_prefix = ""
    formatted_feats_layer = format_layer_features(
        feats_per_layer, filename_prefix, save=save
    )
...
def get_features(
    ...
    if metric == "relative_difference":
        ...
        scores_0 = (sae_acts["known"].mean(dim=0) - sae_acts["unknown"].mean(dim=0)) / (
            sae_acts["unknown"].mean(dim=0) + eps
        )
        scores_1 = (sae_acts["unknown"].mean(dim=0) - sae_acts["known"].mean(dim=0)) / (
            sae_acts["known"].mean(dim=0) + eps
        )
    elif metric == "t_test":
        scores_0 = []
        scores_1 = []
        for i in range(0, sae_acts["known"].shape[1]):
            if (
                sae_acts["known"][:, i].sum() == 0
                and sae_acts["unknown"][:, i].sum() == 0
            ):
                scores_0.append(0.0)
                scores_1.append(0.0)
            else:
    ...
    for known_label, scores in zip([label_0, label_1], [scores_0, scores_1]):
        ...
        freq_acts_dict[known_label] = (
            freq_acts_0.cpu().tolist(),
            freq_acts_1.cpu().tolist(),
        )
        mean_features_acts[known_label] = (
            sae_acts[known_label].mean(0).detach().cpu().tolist()
        )
...
def format_layer_features(feats_per_layer, filename_prefix, save=False):
    ...
    for layer in feats_per_layer.keys():
        ...
        scores_dict, freq_acts_dict, mean_features_acts_dict = feats_per_layer[
            layer
        ]  # Dynamically determine which labels to process (e.g. ['known', 'unknown'] or ['unbias', 'bias'])
        ...
        for known_label in labels:
            ...
            scores, freq_acts, mean_features_acts = (
                scores_dict[known_label],
                freq_acts_dict[known_label],
                mean_features_acts_dict[known_label],
            )
            ...
            for i, idx in enumerate(ordered_indices):
                final_feats_layer_dict[known_label][str(i)] = {
                    "layer": layer,
                    "latent_idx": idx,
                    "score": scores[idx],
...
def read_layer_features(
    model_alias, layers, entity_type, scoring_method, tokens_to_cache, evaluate_on
):
    """
    ...
    for known_label in labels:
        ...
        for layer in feats_layers.keys():
            ...
            for idx in layer_known_label_dict.keys():
                ...
                full_top_freq_acts_0.append(
                    layer_known_label_dict[idx]["freq_acts_known"]
                )
                full_top_freq_acts_1.append(
                    layer_known_label_dict[idx]["freq_acts_unknown"]
                )
                full_top_mean_features_acts.append(
                    layer_known_label_dict[idx]["mean_features_acts"]
                )
        ...
        for i, idx in enumerate(top_k_indices):
            final_feats_dict[known_label][i] = {
                "layer": full_layers_list[idx],
                "latent_idx": full_latent_ids[idx],
                "score": full_scores_array[idx],
                "freq_acts_known": full_top_freq_acts_0[idx],
                "freq_acts_unknown": full_top_freq_acts_1[idx],
...
def plot_top_k_features(final_feats_dict, entity_type):
    ...
    colors = {
        ...
        "unknown": html_colors["red_drawio"],
    }
    ...
    for label in ["known", "unknown", "bias", "unbias"]:
        ...
        for feature_id in final_feats_dict[label].keys():
            ...
            texts.append(
                plt.text(
                    final_feats_dict[label][feature_id]["freq_acts_known"] * 100,
                    final_feats_dict[label][feature_id]["freq_acts_unknown"] * 100,
                    text_feature_id,
                )
            )
    ...
    plt.title(
        f"Feature Activation Frequencies: Known vs Unknown, Entity: {entity_type}"
    )
...
def plot_all_features(
    final_feats_dict, train_feats_dict, entity_type, k=10, labels=True
):
    from matplotlib import patches
    ...
    for label in label_names:
        layer_feat_top_k_training = [
            (
                train_feats_dict[label][i]["layer"],
                train_feats_dict[label][i]["latent_idx"],
            )
            for i in list(train_feats_dict[label].keys())[:k]
        ]
        x = []
        y = []
        x_top = []
        ...
        for i, feature_id in enumerate(final_feats_dict[label].keys()):
            ...
            # if (final_feats_dict[label][feature_id]['layer'], final_feats_dict[label][feature_id]['latent_idx']) in layer_feat_top_k_training and i < k:
            if (
                final_feats_dict[label][feature_id]["layer"],
                final_feats_dict[label][feature_id]["latent_idx"],
            ) in layer_feat_top_k_training:
                x_top.append(x_)
                y_top.append(y_)
                text_feature_id = f'L{final_feats_dict[label][feature_id]["layer"]}F{final_feats_dict[label][feature_id]["latent_idx"]}'
                if labels:
    ...
    plt.title(
        f"Activation Frequencies of SAE Latents: {entity_type.capitalize()}",
        fontsize=10,
    )
...
def plot_heads_heatmaps(mean_attn, title):
    ...
    # Update layout
    fig.update_layout(
        title=title, xaxis_title="Heads", yaxis_title="Layers", width=600, height=400
    )
...
def plot_heads_boxplot_entity_types(
    attn_heads_A, attn_heads_B, A_label, B_label, heads, head_colors, title
):
...
def plot_heads_scatter_entity_types(
    attn_heads_A, attn_heads_B, A_label, B_label, heads, head_colors, title
):
    fig = go.Figure()
    ...
    for j, entity_type in enumerate(entity_types):
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers",
                marker=dict(
                    size=10, symbol=entity_shapes[j % len(entity_shapes)], color="black"
                ),
                name=entity_type,
                showlegend=True,
            )
...
def plot_heads_boxplot(attn_heads_A, attn_heads_B, A_label, B_label, heads, title):
    ...
    # head_colors = ['#1f77b4', '#ff7f0e', '#2ca02c']  # Blue, Orange, Green
    head_colors = [
        html_colors["green_drawio"],
        html_colors["red_drawio"],
    ]  # Blue, Orange, Green, Red
...
def get_logit_diffs_res_stream(
    model,
    base_tokens,
    src_tokens,
    answer_token_indices,
    analysis_type=Literal["accum_res", "per_layer_res", "attn_heads"],
    batch_size=8,
):
    ...
    for i in range(0, len(base_tokens), batch_size):
        ...
        if analysis_type == "accum_res":
            ...
            tensor_to_get_logit_diff = einops.rearrange(
                tensor_to_get_logit_diff,
                "batch n_layers n_heads d_model -> (n_layers n_heads) batch d_model",
            )

        layer_logit_diffs = residual_stack_to_logit_diff(
            model,
            tensor_to_get_logit_diff,
            base_cache,
            logit_diff_directions,
...
def compute_metric(model, base_tokens, src_tokens, answer_token_indices, batch_size=16):
    ...
    for i in range(0, len(base_tokens), batch_size):
        ...
        base_logit_diff = get_logit_diff(
            base_logits.detach(), answer_token_indices_batch, mean=False
        )
        src_logit_diff = get_logit_diff(
            src_logits.detach(), answer_token_indices_batch, mean=False
        )
...
def compute_patching(
    ...
    base_tokens,
    src_tokens,
    patching_type: Literal["resid_streams", "heads_all_pos", "heads_last_pos", "full"],
    answer_token_indices,
    metric,
    batch_size=16,
):
    ...
    for i in range(0, len(base_tokens), batch_size):
        ...
        if patching_type == "resid_streams":
            ...
            patch_results = patching.get_act_patch_resid_pre(
                model, src_tokens_batch, base_cache, metric_fn
            )
            ...
            attn_head_out_per_pos_patch_results = (
                patching.get_act_patch_attn_head_out_by_pos(
                    model, src_tokens_batch, base_cache, metric_fn
                )
            )
            ...
            patch_results = patching.get_act_patch_block_every(
                model, src_tokens_batch, base_cache, metric_fn
            )
    ...
    total_resid_pre_act_patch_results = torch.stack(
        list_resid_pre_act_patch_results, 0
    ).mean(0)
...
def visualize_attention_patterns(
    model,
    type_pattern: Literal[
        "attn_weights", "value_weighted", "output_value_weighted", "distance_based"
    ],
    ...
    for head in heads:
        ...
        if type_pattern == "attn_weights":
            ...
            if type_pattern == "value_weighted":
                ...
                type_pattern == "output_value_weighted"
                or type_pattern == "distance_based"
            ):
                ...
                if type_pattern == "output_value_weighted":
                    ...
                    # weighted_values_norm -> [batch query_pos key_pos head_index]
                    raw_inter_token_attribution = torch.norm(
                        output_weighted_values, dim=-1, p=1
                    )
...
def bar_latents_effect_plot(
    logit_diffs_results: List[Tensor], xticks_labels: List[str], title: str
):
    # Custom colors
    colors = [
        html_colors["blue_matplotlib"],
        html_colors["orange_matplotlib"],
        html_colors["brown_D3"],
    ]
    logit_diff_results = [
        logit_diff_result.cpu().numpy() for logit_diff_result in logit_diffs_results
    ]
    ...
    data = {"Condition": xticks_labels, "Mean": means, "Error": errors}
...
def load_latents(model_alias, top_latents, filter_with_pile=False, **kwargs):
    ...
    if filter_with_pile == True:
        with open(
            f"./train_latents_layers_entities/absolute_difference/{model_alias}/entity/pile_filtered_scores_min_known.json",
            "r",
        ) as f:
            sorted_scores_known = json.load(f)
        with open(
            f"./train_latents_layers_entities/absolute_difference/{model_alias}/entity/pile_filtered_scores_min_unknown.json",
            "r",
        ) as f:
            sorted_scores_unknown = json.load(f)
    else:
        with open(
            f"./train_latents_layers_entities/absolute_difference/{model_alias}/entity/sorted_scores_min_known.json",
            "r",
        ) as f:
            sorted_scores_known = json.load(f)
        with open(
            f"./train_latents_layers_entities/absolute_difference/{model_alias}/entity/sorted_scores_min_unknown.json",
            "r",
        ) as f:
            sorted_scores_unknown = json.load(f)
    # Known latent
    ...
    known_latent: List[Tuple[int, float, Tensor]] = load_steering_latents(
        "movie",
        label="unknown",
        topk=1,
    ...
    unknown_latent: List[Tuple[int, float, Tensor]] = load_steering_latents(
        "movie",
        label="unknown",
        topk=1,
    ...
    random_latents_known: List[Tuple[int, float, Tensor]] = load_steering_latents(
        "movie",
        label="known",
    ...
    random_latents_unknown: List[Tuple[int, float, Tensor]] = load_steering_latents(
        "movie",
        label="unknown",
        topk=kwargs["random_n_latents"],
...
def load_steering_latents_bias(
    ...
    print(
        f"Loading steering latents for model: {model_alias}, Label: {label}, Random: {random_latents}"
    )
    ...
    if specific_latents is not None:
        ...
        for layer, latent_idx in specific_latents:
            found_key = None
            for k_ranked, info in all_sae_latents_dict[
                label
            ].items():  # Iterate over the ranked dictionary
            ...
            if found_key is not None:
                ...
                print(
                    f"Warning: Specific latent (L{layer}F{latent_idx}) not found for label {label}."
                )
        sae_latent_dict = {
            i: all_sae_latents_dict[label][idx] for i, idx in enumerate(indices)
        }
    ...
    if random_latents:
        ...
        try:
            ...
            print(
                f"Warning: `sorted_scores_min_{label}.json` not found at {min_max_scores_path}. Cannot filter random latents by score."
            )
        ...
        for ranked_idx, info in all_sae_latents_dict[label].items():
            ...
            if (
                latent_id_str in min_max_scores
                and abs(min_max_scores[latent_id_str]) > 0.0
            ):
                indices_to_remove.add(
                    ranked_idx
                )  # Remove by the ranked index if it has a score

        available_indices_filtered = [
            idx for idx in available_indices_ranked if idx not in indices_to_remove
        ]

        if len(available_indices_filtered) < topk:
            print(
                f"Warning: Not enough random latents available ({len(available_indices_filtered)}) for topk={topk}. Taking all available."
            )
        ...
        sae_latent_dict = {
            i: all_sae_latents_dict[label][idx]
            for i, idx in enumerate(chosen_indices_ranked)
        }
    ...
    layers_to_process = sorted(
        list(set([l["layer"] for l in sae_info_to_load]))
    ...
    for layer in layers_to_process:
        ...
        if not repo_id:
            raise ValueError(
                f"repo_id not defined for model_alias: {model_alias}. Add it to model_alias_to_sae_repo_id."
            )
        ...
        if model_alias == "gemma-2b-it":
            ...
            model_alias in layer_sparisity_widths
            and (layer - 1) in layer_sparisity_widths[model_alias]
        ):
            ...
            if sae_sparsity is not None:
                sae_id = (
                    f"layer_{layer-1}/width_{sae_width}/average_l0_{str(sae_sparsity)}"
                )
            else:
                print(
                    f"Warning: Sparsity for layer {layer-1} width {sae_width} not found for {model_alias}. Using default SAE ID format."
                )
                sae_id = f"layer_{layer-1}/width_{sae_width}"
        else:
            print(
                f"Warning: Model {model_alias} or layer {layer-1} not in layer_sparisity_widths. Using general SAE ID format."
            )
            sae_id = (
                f"layer_{layer-1}/width_{sae_width}"  # Fallback for other models/layers
            )

        if not sae_id:
            raise ValueError(
                f"SAE ID could not be determined for model {model_alias}, layer {layer}. Check SAE configuration."
            )
        ...
        for latent_info in sae_info_to_load:
            if latent_info["layer"] == layer:
                ...
                mean_act = latent_info["mean_act"]
                direction = (
                    sae.W_enc[:, latent_idx].detach()
                    if input_latent
                    else sae.W_dec[latent_idx].detach()
                )
                latents.append((layer, latent_idx, mean_act, direction))
...
def load_latents_bias(
    ...
    if filter_with_pile == True:
        ...
        with open(
            f"./train_latents_layers_bias/absolute_difference/{model_alias}/bias/pile_filtered_scores_min_unbias.json",
            "r",
        ) as f:
            sorted_scores_unbias = json.load(f)
    else:
        with open(
            f"./train_latents_layers_bias/absolute_difference/{model_alias}/bias/sorted_scores_min_bias.json",
            "r",
        ) as f:
            sorted_scores_bias = json.load(f)
        with open(
            f"./train_latents_layers_bias/absolute_difference/{model_alias}/bias/sorted_scores_min_unbias.json",
            "r",
        ) as f:
            sorted_scores_unbias = json.load(f)
    ...
    unbias_latent: List[Tuple[int, float, Tensor]] = load_steering_latents_bias(
        category,
        label="unbias",
        topk=k,
    ...
    random_latents_bias: List[Tuple[int, float, Tensor]] = load_steering_latents_bias(
        category,
        label="bias",
        topk=kwargs["random_n_latents"],
    ...
    random_latents_unbias: List[Tuple[int, float, Tensor]] = load_steering_latents_bias(
        category,
        label="unbias",
        topk=kwargs["random_n_latents"],
