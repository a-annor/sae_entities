...
def get_acts_labels_dict_(model_alias, tokenizer, dataloader, sae_layers, **kwargs):
    ...
    if "wikidata" in dataset_name:
        ...
        known_entities = [
            q["completion"][-1] for q in queries if q["bias_cat"] == "unbias"
        ]
        unknown_entities = [
            q["completion"][-1] for q in queries if q["bias_cat"] == "bias"
        ]
...
def get_per_layer_latent_scores(
    ...
    if dataloader is None:
        print(
            f"Skipping feature extraction for {dataset_name} — no cached activations found."
        )
        return
...
def scatter_plot_latent_separation_scores_experiment(
    ...
    if tokens_to_cache == "entity":
        ...
        if dataloader is None:
            print(
                f"Skipping feature extraction for 'wikidata_{entity_type}' — no cached activations found."
            )
            return
        ...
        if dataloader is None:
            print(
                f"Skipping feature extraction for bias_{entity_type}' — no cached activations found."
            )
            return
        test_acts_labels_dict = get_acts_labels_dict_(
            model_alias,
        ...
        test_final_feats_dict = get_top_k_features(
            test_feats_layers, k=None, type="bias"
        )
...
def plot_layerwise_latent_scores(
    ...
    for i, entity_type in enumerate(entity_types):
        ...
        if not np.all(np.isnan(scores)):
            error_y = [
                score - min_score for score, min_score in zip(scores, min_scores)
            ]
            error_y_minus = [
