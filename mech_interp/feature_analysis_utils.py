...
def get_dataloader(
    ...
    """
    if "bias" in dataset_name:
        # For bias data, use the actual shard size from the folder name
        category = "_".join(dataset_name.split("_")[1:])
        foldername = f"{DEFAULT_CACHE_DIR}/entity/{model_alias}_{dataset_name}/entity_npositions_1_shard_size_*"
        matching_folders = glob.glob(foldername)
        if not matching_folders:
            raise FileNotFoundError(f"No cached activations found for {dataset_name}")
        # Get the actual shard size from the folder name
        shard_size = int(matching_folders[0].split("shard_size_")[-1])
    elif "pile" in dataset_name:
        # For Pile dataset, use the actual shard size from the folder name
        foldername = f"{DEFAULT_CACHE_DIR}/random/{model_alias}_pile/random_npositions_1_shard_size_*"
        matching_folders = glob.glob(foldername)
        if not matching_folders:
            raise FileNotFoundError(f"No cached activations found for {dataset_name}")
        # Get the actual shard size from the folder name
...
def get_acts_labels_dict_(model_alias, tokenizer, dataloader, sae_layers, **kwargs):
    ...
    if "bias" in dataset_name:
        ...
        # Get the actual shard size from the dataloader
        shard_size = len(dataloader.dataset)

        # Load bias labels from the saved file
        foldername = f"{DEFAULT_CACHE_DIR}/entity/{model_alias}_{dataset_name}/entity_npositions_1_shard_size_{shard_size}"
        with open(f"{foldername}/bias_labels.json", "r") as f:
    ...
    if evaluate_on == "prompts":
        if "wikidata" in dataset_name:
            ...
            known_entities, unknown_entities = get_known_unknown_splits(
                queries, return_type="entity", split=split, consider_refusal_label=False
            )
        ...
        known_prompts, unknown_prompts = get_known_unknown_splits(
            queries,
            return_type="prompt",
            split=None,
            consider_refusal_label=consider_refusal_label,
        )
    ...
    if evaluate_on == "prompts":
        acts_labels_dict = get_acts_labels_dict(
            model_alias,
            tokenizer,
            dataloader,
            queries,
...
def get_per_layer_latent_scores(
    ...
    get_features_layers(
        model_alias, acts_labels_dict, sae_layers, SAE_WIDTH, repo_id, save, **kwargs
    )
...
def scatter_plot_latent_separation_scores_experiment(
    ...
    test_feats_layers = get_features_layers(
        model_alias,
        test_acts_labels_dict,
        testing_layers,
        SAE_WIDTH,
        repo_id,
        save,
    ...
    fig = plot_all_features(
        test_final_feats_dict, train_feats_dict, entity_type, k=10, labels=False
    )
    os.makedirs("./plots/scatter_plots", exist_ok=True)
    fig.savefig(
        f"./plots/scatter_plots/{model_alias}_feature_activation_frequencies_smaller_{entity_type}_{str(testing_layers)}.png",
        transparent=True,
    )
    fig.savefig(
        f"./plots/scatter_plots/{model_alias}_feature_activation_frequencies_smaller_{entity_type}_{str(testing_layers)}.pdf",
        transparent=True,
    )
    fig.show()
...
def get_general_latents(
    ...
    for known_label in ["known", "unknown"]:
        ...
        sorted_scores_mean = dict(
            sorted(scores_mean.items(), key=lambda item: item[1], reverse=True)
        )
        sorted_ranks_mean = dict(
            sorted(ranks_mean.items(), key=lambda item: item[1], reverse=False)
        )
        sorted_scores_min = dict(
            sorted(scores_min.items(), key=lambda item: item[1], reverse=True)
        )

        save_dir = f"./train_latents_layers_{evaluate_on}/{scoring_method}/{model_alias}/{tokens_to_cache}"
        print("saving in", save_dir)
        os.makedirs(save_dir, exist_ok=True)
        for sorted_dict in zip(
            [sorted_scores_min, sorted_scores_mean, sorted_ranks_mean],
            ["scores_min", "scores_mean", "ranks_mean"],
        ):
            if filter_with_pile:
                final_sorted_dict = {}
                counter = 0
                feats_layers_pile = read_layer_features(
                    model_alias,
                    testing_layers,
                    "pile",
                    scoring_method,
                ...
                for latent_id in list(sorted_dict[0].keys()):
                    ...
                    for idx in feats_layers_pile[layer_latent]["unknown"].keys():
                        if (
                            feats_layers_pile[layer_latent]["unknown"][idx][
                                "latent_idx"
                            ]
                            == latent_idx
                        ):
                            pile_freq_score = feats_layers_pile[layer_latent][
                                "unknown"
                            ][idx]["freq_acts_unknown"]
                            break
                ...
                with open(
                    f"{save_dir}/pile_filtered_{sorted_dict[1]}_{known_label}.json", "w"
                ) as f:
                ...
                # Save sorted_scores_min to JSON
                with open(
                    f"{save_dir}/sorted_{sorted_dict[1]}_{known_label}.json", "w"
                ) as f:
                    json.dump(sorted_dict[0], f, indent=4)

        print(
            f"Saved feature analysis results for {known_label} entities to JSON files."
        )
    ...
    for known_label in ["known", "unknown"]:
        ...
        for layer in feats_layers.keys():
            ...
            for full_latent_id in scores[known_label][layer].keys():
                scores_min[known_label][layer][full_latent_id] = np.min(
                    scores[known_label][layer][full_latent_id]
                )
        ...
        for layer in scores_min[known_label].keys():
            for i, (latent_id, _) in enumerate(scores_min[known_label][layer].items()):
                min_score_of_top_k_rank[layer].append(
                    scores_min[known_label][layer][latent_id]
                )
            minmax_layerwise_scores[known_label][layer] = np.max(
                min_score_of_top_k_rank[layer]
            )
...
def plot_layerwise_latent_scores(
    ...
    colors = [
        html_colors["blue_drawio"],
        html_colors["grey_drawio"],
    ...
    for i, entity_type in enumerate(entity_types):
        scores = [
            np.mean(top_scores_layers[known_label][entity_type][layer])
            for layer in sae_layers
        ]
        min_scores = [
            min(top_scores_layers[known_label][entity_type][layer])
            for layer in sae_layers
        ]
        max_scores = [
            max(top_scores_layers[known_label][entity_type][layer])
            for layer in sae_layers
        ]
        error_y = [score - min_score for score, min_score in zip(scores, min_scores)]
        error_y_minus = [
            max_score - score for score, max_score in zip(scores, max_scores)
        ]
