...
def run_generations(model, N, tokenized_prompts, max_new_tokens, batch_size=8):
    ...
    for i in range(0, len(tokenized_prompts), batch_size):
        ...
        generations = model.generate(
            batch_tokenized_prompts, max_new_tokens=max_new_tokens, do_sample=False
        )
        original_generations_full.extend(
            [model.to_string(generation) for generation in generations]
        )
...
def plot_counter_refusal(counter_refusal, save, **kwargs):
    counter_refusal_category_to_plot_label = {
        "original": "Original generation",
        "steered_known": "Steering known latent",
        "steered_unknown": "Steering unknown latent",
        "orthogonalized_unknown": "Orthogonalized model\nUnknown latent",
        "steered_known_random": "Random steering\nKnown latent setting",
        "steered_unknown_random": "Random steering\nUnknown latent setting",
    }

    colors_dict = {
        "original": html_colors["blue_matplotlib"],
        "steered_known": html_colors["dark_green_drawio"],
        "steered_unknown": html_colors["brown_D3"],
        "orthogonalized_unknown": html_colors["orange_drawio"],
        "steered_known_random": html_colors["dark_green_drawio"],
        "steered_unknown_random": html_colors["brown_D3"],
    }

    hatch_dict = {
        "original": None,
        "steered_known": None,
        "steered_unknown": None,
        "orthogonalized_unknown": "///",
        "steered_known_random": "..",
        "steered_unknown_random": "..",
    }
    ...
    for category in categories:
        category_counts[category] = [
            counter_refusal[et][category] for et in entity_types
        ]
    ...
    bar_positions = (
        np.arange(len(entity_types)) * 1.5
    ...
    for cat_idx, category in enumerate(categories):
        ...
        ax.bar(
            bar_positions + offsets[cat_idx],
            values,
            width,
            yerr=yerrs,
            label=counter_refusal_category_to_plot_label[category],
            color=colors_dict[category],
            alpha=1,
            edgecolor="black",
            linewidth=1,
            hatch=hatch_dict[category],
        )
    ...
    if save == True:
        plt.savefig(
            f"./plots/refusal_analysis/v2_{kwargs['model_alias']}_k{kwargs['top_latents']['known']}_u{kwargs['top_latents']['unknown']}_{kwargs['pos_type']}_k{kwargs['coeff_values']['known']}_u{kwargs['coeff_values']['unknown']}_{kwargs['split']}_{kwargs['known_label']}.png",
            dpi=500,
            transparent=True,
        )
        plt.savefig(
            f"./plots/refusal_analysis/v2_{kwargs['model_alias']}_k{kwargs['top_latents']['known']}_u{kwargs['top_latents']['unknown']}_{kwargs['pos_type']}_k{kwargs['coeff_values']['known']}_u{kwargs['coeff_values']['unknown']}_{kwargs['split']}_{kwargs['known_label']}.pdf",
            transparent=True,
        )
...
def count_refusals(generations):
    ...
    for generation in generations:
        ...
        generation = generation[
            pos_end_of_instruction + len(end_of_instruction_string) :
        ]
...
for entity_type in ["player", "movie", "song", "city"]:
    ...
    for known_label in ["unknown", "known"]:
        (
            tokenized_prompts_dict_entity_type[entity_type][known_label],
            pos_entities_dict_entity_type[entity_type][known_label],
            formatted_instructions_dict_entity_type[entity_type][known_label],
            _,
        ) = load_data(
            model,
            queries,
...
known_latent, unknown_latent, random_latents_known, random_latents_unknown = (
    load_latents(model_alias, top_latents, random_n_latents=10, filter_with_pile=True)
)
...
for e_idx, entity_type in enumerate(["player", "city", "movie", "song"]):
    ...
    if e_idx > 0:
        if (
            "orthogonalized_unknown" in categories
            or "orthogonalized_known" in categories
        ):
    ...
    tokenized_prompts, pos_entities, formatted_instructions = (
        tokenized_prompts_dict_entity_type[entity_type][known_label],
        pos_entities_dict_entity_type[entity_type][known_label],
        formatted_instructions_dict_entity_type[entity_type][known_label],
    )
    ...
    if "original" in categories:
        original_generations_full, known_steered_generations_full = (
            steered_and_orig_generations(
                model,
                N,
                tokenized_prompts,
                pos_entities,
                ...
                ablate_latents=None,
                coeff_value=coeff_values["known"],
                max_new_tokens=max_new_tokens,
                batch_size=batch_size,
            )
        )

        counter_refusal[entity_type]["original"] = count_refusals(
            original_generations_full
        )
        counter_refusal[entity_type]["steered_known"] = count_refusals(
            known_steered_generations_full
        )

    if "steered_unknown" in categories:
        _, unknown_steered_generations_full = steered_and_orig_generations(
            model,
            N,
            tokenized_prompts,
            pos_entities,
            ...
            ablate_latents=None,
            coeff_value=coeff_values["unknown"],
            max_new_tokens=max_new_tokens,
            orig_generations=False,
            batch_size=batch_size,
        )

        counter_refusal[entity_type]["steered_unknown"] = count_refusals(
            unknown_steered_generations_full
        )

    if "steered_known_random" in categories:
        random_latents_counter = []
        for random_latent in random_latents_known:
            _, random_steered_generations_full = steered_and_orig_generations(
                model,
                N,
                tokenized_prompts,
                pos_entities,
                pos_type=pos_type,
                steering_latents=[random_latent],
                ablate_latents=None,
                coeff_value=coeff_values["known"],
                max_new_tokens=max_new_tokens,
                orig_generations=False,
            ...
            random_latents_counter.append(
                count_refusals(random_steered_generations_full)
            )
    ...
    if "orthogonalized_unknown" in categories or "orthogonalized_known" in categories:
        for ortho_idx, orhogonalization_type in enumerate(["unknown"]):
            ...
            orthogonalized_generations_full = run_generations(
                model, N, tokenized_prompts, max_new_tokens, batch_size
            )
            torch.cuda.empty_cache()

            counter_refusal[entity_type][f"orthogonalized_{orhogonalization_type}"] = (
                count_refusals(orthogonalized_generations_full)
            )
...
with open(
    os.path.join(save_path, f"counter_refusal_{entity_type}_{known_label}.json"), "w"
) as f:
...
# %%
known_latent, unknown_latent, random_latents_known, random_latents_unknown = (
    load_latents(model_alias, top_latents, filter_with_pile=True, random_n_latents=10)
)
# %%
...
original_generations_full, steered_generations_full = steered_and_orig_generations(
    model,
    N,
    tokenized_prompts,
    pos_entities,
    pos_type="entity_last_to_end",
    steering_latents=[random_latents_known[-2]],
    ablate_latents=None,
    coeff_value=coeff_values["known"],
    max_new_tokens=max_new_tokens,
...
for i in range(len(original_generations_full)):
    clean_completion = (
        original_generations_full[i]
        .replace(formatted_instructions[i], "")
        .replace("<bos>", "")
        .replace("<pad>", "")
    )
    steered_completion = (
        steered_generations_full[i]
        .replace(formatted_instructions[i], "")
        .replace("<bos>", "")
        .replace("<pad>", "")
    )
    ...
    print(
        textwrap.fill(
            repr(
                steered_completion.replace("<eos>", "").replace("\n<end_of_turn>", "")
            ),
            width=100,
            initial_indent="\t",
            subsequent_indent="\t",
