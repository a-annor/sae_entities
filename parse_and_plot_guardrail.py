...
def plot_guardrail_removals_by_coeff(exp, latent_id, latent_type="sentiment", output_path="guardrail_removals_by_coeff.png"):
    ...
    for col in [f"{label_1}_guardrail_removed", f"{label_2}_guardrail_removed", "original_guardrail_removed"]:
    ...
    df_long = guardrail_counts.melt(
        id_vars="coeff",
        var_name="Steering Type",
        value_name="Guardrail Removals"
    )
    ...
    plt.title(f"Guardrail Removals vs. Steering Coefficient - Latent {latent_id}", fontsize=16, pad=20)
