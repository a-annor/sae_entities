...
def _get_activations_fixed_seq_len(
    ...
    activations = torch.zeros(
        (len(prompts), n_layers, seq_len, d_model), device=save_device
    )
    all_input_ids = torch.zeros(
        (len(prompts), seq_len), dtype=torch.long, device=save_device
    )
        ...
        fwd_pre_hooks = [
            (
                ...
                _get_activations_pre_hook(
                    cache=activations[
                        i : i + inputs_len, layer_idx, -num_input_toks:, :
                    ]
                ),
            )
...
def get_compute_activation_fn(
    ...
    def compute_activation_fn(prompts: List[str]):
        ...
        assert n_layers == len(
            layers
        ), f"Expected {len(layers)} layers, but got {n_layers}"
...
def cache_activations(
    model_base: Any,
    prompts: List[str],
    compute_activation_fn: Callable,
    tokens_to_cache: str,
    layers: List[int],
    foldername: str,
    batch_size: int = 32,
    ...
    memmap_file_ids = np.memmap(
        f"{foldername}/ids.dat", dtype="float32", mode="w+", shape=(shard_size, seq_len)
    )
    ...
    for i in tqdm(range(0, len(prompts), batch_size), desc="Computing activations"):
        ...
        batch_prompts_for_model = [
            prompt.replace("<bos>", "") for prompt in prompts[i : i + batch_size]
        ]
        ...
        for j in range(activations.shape[0]):
            ...
            actual_len_idx = (
                input_ids[j] == model_base.tokenizer.pad_token_id
            ).nonzero(as_tuple=True)[0]
            actual_len = (
                (input_ids[j] != model_base.tokenizer.pad_token_id).sum().item()
            )

            if actual_len == 0:
                # This warning means the prompt tokenized to an empty sequence.
                # only for truly empty or invalid input strings.
                print(
                    f"Warning: Skipping example {i+j} due to empty tokenized sequence (actual_len is 0)."
                )
                continue
            ...
            if current_activations_for_item.shape[1] < n_positions:
                num_missing_positions = (
                    n_positions - current_activations_for_item.shape[1]
                )
                ...
                padded_activations[:, num_missing_positions:, :] = (
                    current_activations_for_item
                )
            ...
            if i == 0 and j == 0:
                ...
                print(
                    "Decoded Full Input (truncated to seq_len):",
                    model_base.tokenizer.decode(
                        input_ids[j], skip_special_tokens=False
                    ),
                )
                print("Actual sequence length (non-padded):", actual_len)
                print(
                    f"Slicing from index {start_pos} to {seq_len} (exclusive). Taking {n_positions} tokens."
                )
                print(
                    "Token(s) that will be cached:",
                    model_base.tokenizer.decode(
                        input_ids[j, start_pos:seq_len], skip_special_tokens=False
                    ),
                )
                print(
                    f"Shape of activations for this item: {current_activations_for_item.shape}"
                )
                print(
                    f"Min/Max/Mean of activations for this item: {current_activations_for_item.min().item():.4f}/{current_activations_for_item.max().item():.4f}/{current_activations_for_item.mean().item():.4f}"
                )
                print("------------------------------------")
        ...
        if not current_batch_activations_sliced:
            ...
            print(
                f"Warning: No valid activations sliced for batch starting at index {i}. Skipping batch."
            )
            continue
        ...
        memmap_file_acts[total_n_cached : total_n_cached + num_to_add] = (
            activations_to_save[:num_to_add].float().numpy()
        )
        memmap_file_ids[total_n_cached : total_n_cached + num_to_add] = input_ids[
            :num_to_add
        ].numpy()
...
def load_triviaqa_instructions_to_cache(
    model_alias: str, tokens_to_cache: str, balanced_data: bool = False
):
    from dataset.load_data import load_triviaqa_queries, balance_data
...
def load_wikidata_instructions_to_cache(
    model_alias: str,
    tokens_to_cache: str,
...
def load_bias_instructions_to_cache(
    ...
    substrings = [
        q["completion"][-1] for q in queries if q["bias_cat"] != "undetermined"
    ]
...
class CachedDataset(TorchDataset):
    ...
    def __init__(
        ...
        input_ids = np.memmap(
            input_ids_path, dtype=dtype, mode="r", shape=(shard_size, seq_len)
        )
...
def main(args):
    ...
    n_positions = (
        1  # starting from the tokens_to_cache, how many positions to take to the left
    )
    ...
    def get_activations(
        ...
        compute_activation_fn = get_compute_activation_fn(
            model_base, seq_len=seq_len, batch_size=batch_size
        )
    ...
    if dataset == "wikidata":
        ...
        for entity_type in ["player", "movie", "city", "song"]:
            ...
            prompts, substrings = load_wikidata_instructions_to_cache(
                model_alias=model_alias,
                tokens_to_cache=tokens_to_cache,
        ...
        for bias_type in ALL_BIAS_TYPES:
            ...
            prompts, substrings = load_bias_instructions_to_cache(
                tokens_to_cache, category=bias_type
            )
            if len(prompts) == 0:
        ...
        prompts, substrings = load_triviaqa_instructions_to_cache(
            model_alias, tokens_to_cache
        )
        get_activations(
            model_base,
            prompts,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="gemma-2-2b",
        help="Alias of the model to use",
    )
    parser.add_argument(
        "--tokens_to_cache",
        type=str,
        default="entity",
        help='How to find the position to cache. Options: "entity", "last_eoi", "?", "random"',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help='Whether to format the prompt as: The {entity_type} "{entity}"',
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="wikidata",
        help='Dataset to use. Options: "wikidata", "triviaqa", "pile", "bias"',
    )
