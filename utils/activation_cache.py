...
def cache_activations(
    ...
    for i in tqdm(range(0, len(prompts), batch_size), desc="Computing activations"):
        ...
        batch_substrings_tmp = (
            substrings[i : i + batch_size] if substrings is not None else None
        )
        # remove bos
        batch_prompts_tmp = [prompt.replace("<bos>", "") for prompt in batch_prompts]

        ### Check if the substrings are in the input ids
        inputs_ = model_base.tokenizer(
            batch_prompts_tmp,
            return_tensors="pt",
            padding=True,
            truncation=True,
        ...
        if tokens_to_cache is not None:
            ...
            for j, s in enumerate(slices_to_cache):
                ...
                if i == 0 and j == 0:
                    print(
                        "tokens",
                        model_base.tokenizer.convert_ids_to_tokens(input_ids[j]),
                    )
                    print(
                        "token(s) cached:",
                        model_base.tokenizer.decode(input_ids[j, left_pos : s.stop]),
                    )
...
def load_bias_instructions_to_cache(
    ...
    data_path = f"z_my_data/test_prompt_final/test_{category}_completion_sentiment_judged_final.jsonl"
    ...
    return prompts, substrings, bias_labels
...
class CachedDataset(TorchDataset):
    ...
    def __init__(
        ...
        # Extract shard size from path_name if not provided
        if shard_size is None:
            # Path format: .../shard_size_{shard_size}
            shard_size = int(path_name.split("shard_size_")[-1])
...
def main(args):
    ...
    # Define shard sizes for different datasets
    ...
    def get_activations(
        ...
        if "wikidata" in dataset_name:
            ...
            # For bias data, get the full category name (e.g., "Race_ethnicity")
        ...
        # Save bias labels if they exist
        if bias_labels is not None:
            with open(f"{foldername}/bias_labels.json", "w") as f:
    ...
    if dataset == "bias":
        ...
        for category in ALL_BIAS_TYPES:
            dataset_name = f"bias_{category}"
            prompts, substrings, bias_labels = load_bias_instructions_to_cache(
                model_alias=model_alias,
...
if __name__ == "__main__":
    ...
    Bias activations: python -m utils.activation_cache --model_alias gemma-2-2b --tokens_to_cache entity --batch_size 128 --dataset bias
    """
